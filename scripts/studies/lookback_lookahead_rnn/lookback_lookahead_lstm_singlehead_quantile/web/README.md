...
For the current study `OBSERVATION_SET_NAME = "lookback_lookahead_lstm_singlehead_quantile"`,
so the viewer is served at:

```
https://storage.googleapis.com/payamdpycryptoreports/lookback_lookahead_lstm_singlehead_quantile/app/index.html
```
...
4. Hardcoded fallbacks (`payamdpycryptoreports` /
   `lookback_lookahead_lstm_singlehead_quantile`).
...
`lookback_lookahead_lstm_singlehead_quantile/app/`). The `WEB_DIR` is resolved from the
