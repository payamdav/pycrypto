...
For the current study `OBSERVATION_SET_NAME = "lookback_lookahead_mlp_quantile"`,
so the viewer is served at:

```
https://storage.googleapis.com/payamdpycryptoreports/lookback_lookahead_mlp_quantile/app/index.html
```
...
4. Hardcoded fallbacks (`payamdpycryptoreports` /
   `lookback_lookahead_mlp_quantile`).
...
```
python scripts/studies/lookback_lookahead_mlp/lookback_lookahead_mlp_quantile/upload_web_app.py
```

This uploads to `{OBSERVATION_SET_NAME}/app/` (default
`lookback_lookahead_mlp_quantile/app/`). The `WEB_DIR` is resolved from the
...
```
python scripts/studies/lookback_lookahead_mlp/lookback_lookahead_mlp_quantile/upload_web_app.py --prefix OtherStudy/app
```
