#!/usr/bin/env python3
"""
Lookback / Lookahead MLP study (Quantile + MSE) — MULTIPROCESSING trainer.
...
Concurrency model:
    * Phase 1 — Load ALL data once, IN THE PARENT PROCESS.
    * Phase 2 — Spawn a pool of ``--concurrency N`` worker processes.
    * Phase 3 — The 14 observation specs are mapped across the pool.
...
# --------------------------------------------------------------------------- #
#  Worker process: init + per-observation work
# --------------------------------------------------------------------------- #
...
def main():
    parser = argparse.ArgumentParser(
        description=("Lookback/Lookahead MLP sweep (Quantile + MSE) — "
