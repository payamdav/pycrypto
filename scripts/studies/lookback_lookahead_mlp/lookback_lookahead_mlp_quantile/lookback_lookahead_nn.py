#!/usr/bin/env python3
"""
Lookback / Lookahead single-target MLP regression study (Quantile transform + MSE loss).

This script trains a compact MLP for every combination of:

    7 assets  x  2 label varieties (l_e_vwap[0] and l_e_vwap[3])
        => 14 observations total.

Input features (8 scalars, flat vector — no sequence dimension):
    f_e_vwap[-3], f_e_vwap[-2], f_e_vwap[-1]   (last 3 of the 24-step window)
    f_e_n_imbalances[-2], f_e_n_imbalances[-1]  (last 2 of the 24-step window)
    f_rsi_1_1_p1  (RSI period 7),  f_rsi_1_1_p2 (period 14),
    f_rsi_1_1_p3  (period 60)

Label varieties (single target, bounded in [-1, 1]):
    - l_e_vwap[0]  (1h horizon)
    - l_e_vwap[3]  (4h horizon)

Label transform: QuantileTransformer (uniform) fitted on train split, scaled to [-1, 1].
Predictions are inverse-transformed back to the original label space before evaluation.

For every observation the script:
    1. Loads the fl_data_{asset} numpy array from local ~/data cache or GCS.
    2. Builds a chronological 70/15/15 train/val/test split with a purge gap.
    3. Fits a quantile transform on the train labels.
    4. Trains the MLP (two hidden layers: 16 → 8 units, ReLU, Dropout, tanh output).
    5. Computes the full Eval_/Report_ metrics suite from the evaluation blueprint.
    6. Packages everything into the master JSON schema (Plotly-only visualizations).
    7. Uploads the report (and the trained model params) to the study's
       reports/ subfolder:
...
OBSERVATION_SET_NAME = "lookback_lookahead_mlp_quantile"
MODEL_TYPE = "MLP_16_8_SingleHead_Quantile"
LOSS_NAME = "MSE Loss"
...
# RSI columns (indices 49, 50, 51)
F_RSI_P1_I = 49   # RSI period 7
F_RSI_P2_I = 50   # RSI period 14
F_RSI_P3_I = 51   # RSI period 60

# Feature selection: 8 scalar columns from fl_data
# f_e_vwap[-3]=col22, f_e_vwap[-2]=col23, f_e_vwap[-1]=col24
# f_e_n_imbalances[-2]=col47, f_e_n_imbalances[-1]=col48
# f_rsi_1_1_p1=col49, f_rsi_1_1_p2=col50, f_rsi_1_1_p3=col51
FEATURE_COLS = [22, 23, 24, 47, 48, 49, 50, 51]
FEATURE_NAMES = [
    "f_e_vwap[-3]",
    "f_e_vwap[-2]",
    "f_e_vwap[-1]",
...
# Only l_e_vwap[0] (1h) and l_e_vwap[3] (4h)
LABEL_INDICES = [0, 3]
...
# When True the training fit loop is skipped (CPU smoke-test / dry run).
NO_TRAIN = False
...
class MLPRegressor(nn.Module):
    """Compact 2-layer MLP with a single tanh-bounded regression output.

    Input: (batch, 8) flat feature vector.
    Output: (batch,) scalar in (-1, 1).
    """
    ...
    def forward(self, x):
        # x shape: (batch, 8)
        return torch.tanh(self.net(x)).squeeze(-1)  # (batch,)


def build_model():
    """Construct the study's MLP regressor."""
    model = MLPRegressor()
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  Model: MLPRegressor | parameters: {n_params}", flush=True)
    return model
...
def load_fl_data(asset: str) -> np.ndarray:
    """Load fl_data_{asset} as a float64 array.

    Reads from the local ~/data cache when present. If the local file is
    missing, downloads the blob from GCS, writes it to the cache, and loads it.
    """
...
def build_features(fl: np.ndarray) -> np.ndarray:
    """Return flat feature matrix of shape (n_obs, 8).

    Selects the 8 scalar feature columns from fl_data:
      f_e_vwap[-3], f_e_vwap[-2], f_e_vwap[-1],
      f_e_n_imbalances[-2], f_e_n_imbalances[-1],
      f_rsi_1_1_p1, f_rsi_1_1_p2, f_rsi_1_1_p3
    """
    return fl[:, FEATURE_COLS].astype(np.float64)


def build_labels(fl: np.ndarray, label_idx: int) -> np.ndarray:
    """Return a single l_e_vwap label column (variant 0 or 3), shape (n_obs,)."""
...
def train_model(model, train_loader, val_loader):
    """Train with Adam + MSE loss + ReduceLROnPlateau + early stopping.
...
def run_observation(asset, label_idx, fl):
    ...
    # Fit the quantile label transform on the TRAIN-split labels only.
    # Transform train/val (training + early stopping happen in transformed space).
    # Keep the TEST labels in ORIGINAL space for evaluation.
...
# --------------------------------------------------------------------------- #
#  Main loop: 7 assets x 2 labels = 14 observations
# --------------------------------------------------------------------------- #
def main():
    global NO_TRAIN

    parser = argparse.ArgumentParser(
        description="Lookback/Lookahead single-target MLP sweep (Quantile + MSE).")
    parser.add_argument(
    ...
    total = len(ASSETS) * len(LABEL_INDICES)
    done = 0
    failures = []
    ...
    for asset in ASSETS:
        ...
        for label_idx in LABEL_INDICES:
            ...
            tag = f"{asset}/l_e_vwap{label_idx}"
            print(f"\n[{done}/{total}] {tag}", flush=True)
            try:
                run_observation(asset, label_idx, fl)
            except Exception as exc:  # noqa: BLE001
