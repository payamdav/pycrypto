#!/usr/bin/env python3
"""
Lookback / Lookahead single-target MLP regression study (MSE loss).
...
For every observation the script:
    ...
    3. Trains the MLP (two hidden layers: 16 → 8 units, ReLU, Dropout, tanh output).
...
OBSERVATION_SET_NAME = "lookback_lookahead_mlp_mse"
MODEL_TYPE = "MLP_16_8_SingleHead_MSE"
LOSS_NAME = "MSE Loss"
LABEL_TRANSFORM_NAME = "none"
...
def run_observation(asset, label_idx, fl):
    ...
    # No label transform — raw labels used directly
    ...
    # No inverse transform — predictions are in original label space
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Lookback/Lookahead single-target MLP sweep (MSE loss).")
    parser.add_argument(
