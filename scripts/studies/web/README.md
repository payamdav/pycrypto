...
For the current study `OBSERVATION_SET_NAME = "Lookback_Lookahead_LSTM_Sweep_v1"`,
so the viewer is served at:

```
https://storage.googleapis.com/payamdpycryptoreports/Lookback_Lookahead_LSTM_Sweep_v1/app/index.html
```
...
4. Hardcoded fallbacks (`payamdpycryptoreports` /
   `Lookback_Lookahead_LSTM_Sweep_v1`).
...
`Lookback_Lookahead_LSTM_Sweep_v1/app/`). The `WEB_DIR` is resolved from the
