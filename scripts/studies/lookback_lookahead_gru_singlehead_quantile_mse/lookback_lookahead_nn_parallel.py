#!/usr/bin/env python3
"""
Lookback / Lookahead single-target RNN study — PARALLEL, GPU-resident trainer.
...
Why the results are unchanged (vs. the sequential script):
    * Per-model PROCEDURE is byte-for-byte the same: same model class, same
      AdamW + loss + ReduceLROnPlateau + early-stopping + MAX_EPOCHS, same
...
class Observation:
    ...
    def free_device_tensors(self):
        # NOTE: self.qt is deliberately NOT cleared — the fitted transformer is
        # a small host object reused by report_observation (inverse transform).
...
def main():
    parser = argparse.ArgumentParser(
        description=("Lookback/Lookahead single-target RNN sweep — parallel, "
