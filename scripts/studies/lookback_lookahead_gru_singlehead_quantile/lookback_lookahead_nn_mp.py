#!/usr/bin/env python3
"""
Lookback / Lookahead single-target RNN study — MULTIPROCESSING trainer.
...
Why the results are unchanged (vs. the sequential script):
    * Per-model PROCEDURE is byte-for-byte the same: each worker reuses
      ...
      ``base.build_model``, ``base.make_loader`` and ``base.train_model``
      (same AdamW + loss + ReduceLROnPlateau + early-stopping + MAX_EPOCHS,
...
Per-study choices (RNN cell, loss, model_type, observation set name, label
transform) are funnelled through ``base`` constants/factories, so this file is
IDENTICAL across all four quantile studies.
...
#
#  This file lives in scripts/studies/<study>/, so the repo root is THREE levels
#  up — matching the base module's bootstrap exactly.
# --------------------------------------------------------------------------- #
...
def run_observation_mp(asset, label_idx, input_variety, fl):
    ...
    the base feature/label/split builders, model factory, loaders, train_model,
    ...
    # Fit the quantile label transform on the TRAIN-split labels only, transform
    # train/val (training + early stopping happen in transformed space), and
    ...
    model = base.build_model(n_features)
...
def main():
    parser = argparse.ArgumentParser(
        description=("Lookback/Lookahead single-target RNN sweep — "
