...
For the current study `OBSERVATION_SET_NAME = "lookback_lookahead_lstm_singlehead_quantile_mse"`,
so the viewer is served at:

```
https://storage.googleapis.com/payamdpycryptoreports/lookback_lookahead_lstm_singlehead_quantile_mse/app/index.html
```
...
4. Hardcoded fallbacks (`payamdpycryptoreports` /
   `lookback_lookahead_lstm_singlehead_quantile_mse`).
...
`lookback_lookahead_lstm_singlehead_quantile_mse/app/`). The `WEB_DIR` is resolved from the
